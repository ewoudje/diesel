...
import {playChain} from './dialogue';
...
<template>
  ...
  <!--===MISC UNDERLAYS===-->
  <Group :anchor="{ Left:-50,Bottom:-35,Top:-25,Width: 200, Height: 440 }"
  ...
  <Group :anchor="{Left:-20, Bottom: -50, Height: 100}">
    ...
    <Group :anchor="{ Left:-30,Bottom:0,Width: 1000, Height: 440 }">
      ...
      <!--portrait-->
      <Group :anchor="{Left:60, Bottom:22, Width: 320, Height: 320 }"
             :background="{Color:colors.base}" :mask-texture-path="displayPortrait"></Group>
    </Group>
    ...
    <!--Shine overlay-->
    <Group :anchor="{ Left:-30,Bottom:0,Width: 1000, Height: 440 }"
           :background="'Img/bottom_left_wide_shine.png'"></Group>

    <!--Dashes-->
    <Group :anchor="{Left:350, Bottom:198, Height:100, Width:200}" :background="contentBg"
           :layout-mode="'Left'">
      <Group v-for="(entry, index) in playerClass.maxDashes" :key=index
             :anchor="{Width: 48, Height: 48, Left:20 }" :mask-texture-path="'Img/mask/button.png'">
        <Group
          :background="index < inputDashes ? 'Img/charge_on.png' : 'Img/charge_off.png'"></Group>
      </Group>
    </Group>
  ...
  <Group :anchor="{Right:0}">
    ...
    <!--hotbar-->
    <!--weapons-->
    <Group :anchor="{Right:0, Top:0, Bottom:0, Width:120}" :background="contentBg"
           :layout-mode="'Top'">
      <Group v-for="(entry, index) in playerClass.hotbar" :key="index"
             :anchor="{Left:0, Top:22, Width: 100, Height: 100 }"
             :background="(inputHotbarIdx == index) ? 'Img/button_on_overlay.png' : 'Img/button_off_overlay.png'"
             :mask-texture-path="'Img/mask/button.png'">
        <Group
          :background="{Color:`${(inputHotbarIdx == index) ? colors.base : LightenDarkenColor(colors.base,-100)}(0.5)`}"/>
        <Group
          :background="(inputHotbarIdx == index) ? 'Img/button_on_top_overlay.png' : 'Img/empty.png'"/>
        <Group :anchor="{Width:60,Height:60}"
               :background="(inputHotbarIdx == index) ? {Color:colors.dark} : {Color:colors.light}"
               :mask-texture-path="playerClass.hotbar[index]?.iconPath"/>
        <Group :anchor="{Left:8}">
          <Label
            :anchor="{Left:2}"
            :el-style="{
    ...
    <!--abilities-->
    <Group :anchor="{Right:0, Top:280, Bottom:0, Width:120}" :background="contentBg"
           :layout-mode="'Top'">
      <Group v-for="(entry, index) in heldItem.o?.abilities" :key="index"
             :anchor="{Left:0, Top:22, Width: 100, Height: 100 }"
             :background="false ? 'Img/button_on_overlay.png' : 'Img/button_off_overlay.png'"
             :mask-texture-path="'Img/mask/button.png'">
        <Group :background="{Color:`${colors.alt}(0.3)`}"/>
        <Group :background="false ? 'Img/button_on_top_overlay.png' : 'Img/empty.png'"/>
        <Group :anchor="{Width:60,Height:60}"
               :background="false ? {Color:colors.dark} : {Color:colors.light}"
               :mask-texture-path="heldItem.o?.abilities[index]?.iconPath"/>
        <Group :anchor="{Left:8}">
          <Label
            :anchor="{Left:2}"
            :el-style="{
  ...
  <Group :anchor="{  Right: -20, Bottom: -52}">
    ...
    <Group :anchor="{ Right:250, Bottom:0,Width: 400, Height: 150 }" :background="contentBg">
      <Group :anchor="{Left:-20, Bottom:62, Width: 400, Height: 130 }"
             :background="{Color:colors.base}" :mask-texture-path="heldItem.o?.previewPath"></Group>

    </Group>
    <!--Ammo counter-->
    <Group :anchor="{ Width: 600, Height: 300, Right:-90, Bottom: 30}" :background="contentBg">
      <Group v-if="heldItem.o?.maxAmmo"
             :anchor="{Top:30,Left:(heldItem.ammo.value < 10) ? -15 : 40}">
        <Label
      ...
      <Group v-else :anchor="{Top:-30}" :layout-mode="'Top'">
        <Label
    ...
    <Group :anchor="{Bottom:0,Right:-90, Width:80000,Height:450}"
