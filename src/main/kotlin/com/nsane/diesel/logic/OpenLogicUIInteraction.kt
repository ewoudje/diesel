...
class OpenLogicUIInteraction : SimpleBlockInteraction() {
    ...
    private fun doInteraction(
        ...
        val chunk = world.getChunk(ChunkUtil.indexChunkFromBlock(targetBlock.x, targetBlock.z))?.blockComponentChunk
            ?: return InteractionState.Failed
        val block = chunk.getEntityReference(ChunkUtil.indexBlockInColumn(targetBlock.x, targetBlock.y, targetBlock.z))
            ?: return InteractionState.Failed
        if (!block.isValid) return InteractionState.Failed

        val player = context.commandBuffer?.getComponent(context.entity, Player.getComponentType())
            ?: return InteractionState.Failed
        val playerRef = context.commandBuffer?.getComponent(context.entity, PlayerRef.getComponentType())
            ?: return InteractionState.Failed
