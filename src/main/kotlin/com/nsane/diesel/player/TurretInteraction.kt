...
class TurretInteraction : SimpleBlockInteraction() {
    ...
    private fun isOccupied(world: World, commandBuffer: CommandBuffer<EntityStore>, targetBlock: Vector3i): Boolean =
        world.playerRefs
            ...
            .any {
                commandBuffer.getComponent(it, TransformComponent.getComponentType())!!.position.clone().subtract(
                    targetBlock.x + 0.5,
                    targetBlock.y + 0.5,
                    targetBlock.z + 0.5
                ).length() < 2
    ...
    companion object {
        val CODEC =
            BuilderCodec.builder(TurretInteraction::class.java, ::TurretInteraction, SimpleBlockInteraction.CODEC)
                .build()
    }
}